...
export async function createFromTemplate({ fullAppPath, templateId = DEFAULT_TEMPLATE_ID, }) {
...
export async function initializeAppStructure(appPath, templateId = DEFAULT_TEMPLATE_ID) {
...
async function createDefaultAiRules(aiRulesPath, templateId) {
    let aiRules = "";
    switch (templateId) {
        case "vite-react":
            aiRules = `# Tech Stack
...
async function ensureDevScript(appPath, templateId) {
...
async function cloneRepo(repoUrl) {
    ...
    const cachePath = await path.join(appDataPath, "templates-cache", orgName, repoBasename);
...
async function ensurePostCSSConfig(appPath) {
...
async function copyRepoToApp(_repoCachePath, _appPath) {
