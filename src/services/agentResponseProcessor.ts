...
type Operation = FileOperation | DependencyOperation | CommandOperation | EnvSetupOperation;
...
function parsePrestigeTags(response: string): { operations: Operation[], chatSummary: string, chatContent: string } {
  ...
  const envSetupRegex = /<prestige-setup-env service="([^"]+)" apiKey="([^"]+)"(?:\s+description="([^"]+)")?>([\s\S]*?)<\/prestige-setup-env>/g;
  while ((match = envSetupRegex.exec(response)) !== null) {
    operations.push({ 
      type: 'setup-env', 
      service: match[1], 
      apiKey: match[2], 
    ...
    try {
      switch (op.type) {
        ...
        case 'command':
            ...
            break;
        case 'setup-env':
          // Handle API key setup and .env file management
          try {
            await setupEnvironmentVariable(op.service, op.apiKey, op.description, appPath);
            showSuccess(`🔑 Environment variable ${op.apiKey} set up for ${op.service}`);
          } catch (error) {
            console.error('Environment setup failed:', error);
            showError(`Failed to setup ${op.service}: ${error instanceof Error ? error.message : String(error)}`);
          }
          break;
      }
    } catch (error) {
      ...
      } else if (op.type === 'setup-env') {
        opIdentifier = `${op.service} (${op.apiKey})`;
      }
...
async function setupEnvironmentVariable(
  ...
  const envPath = await path.join(appPath, '.env');
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].startsWith(`${apiKey}=`)) {
        // Update existing key with placeholder
        lines[i] = `${apiKey}=your_${service.toLowerCase()}_api_key_here`;
        keyExists = true;
        break;
      }
    }
    ...
    if (!keyExists) {
      ...
      const newSection = [
        ...
        `${apiKey}=your_${service.toLowerCase()}_api_key_here`,
        ''
      ];
      
      envContent += newSection.join('\n');
    } else {
    ...
    console.log(`✅ Updated .env file with ${apiKey} for ${service}`);
    
  } catch (error) {
    console.error(`❌ Failed to setup environment variable for ${service}:`, error);
    throw new Error(`Failed to setup ${service} environment: ${error instanceof Error ? error.message : String(error)}`);
  }
}
