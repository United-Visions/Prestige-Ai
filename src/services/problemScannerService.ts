import { Problem, ProblemReport } from '../stores/problemsStore';
import { AIProblemDetector } from './aiProblemDetector';

export class ProblemScannerService {
  private static instance: ProblemScannerService;
  private isScanning: boolean = false;
  private scanQueue: string[] = [];
  private lastScanResults: Map<string, Problem[]> = new Map();
  private aiDetector: AIProblemDetector;

  private constructor() {
    this.aiDetector = AIProblemDetector.getInstance();
  }

  public static getInstance(): ProblemScannerService {
   ...
   */
  async scanForProblems(filePath: string): Promise<Problem[]> {
   ...
   */
  async scanMultipleFiles(filePaths: string[]): Promise<ProblemReport> {
   ...
   */
  async detectProblemsInRealTime(code: string, filePath?: string): Promise<Problem[]> {
    try {
   ...
   */
  generateFixSuggestions(problems: Problem[]): string[] {
   ...
   */
  createAutoFixPrompt(problems: Problem[]): string {
    if (problems.length === 0) return 'No problems detected.';
  ...
  private async runTypeScriptCheck(filePath: string): Promise<Problem[]> {
  ...
  private detectSyntaxErrors(code: string): Problem[] {
    const problems: Problem[] = [];
  ...
  private detectTypeErrors(code: string): Problem[] {
    const problems: Problem[] = [];
    const lines = code.split('\n');
  ...
  private detectLintingIssues(code: string): Problem[] {
    const problems: Problem[] = [];
  ...
  private extractIdentifierFromMessage(message: string): string {
  ...
  public getLastResults(filePath: string): Problem[] {
