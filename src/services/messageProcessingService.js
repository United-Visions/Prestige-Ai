export class MessageProcessingService {
     ...
     */
    limitChatHistory(messages, maxChatTurns) {
        ...
        if (messageHistory.length > maxChatTurns * 2) {
            ...
            if (recentMessages.length > 0 && recentMessages[0].role !== 'user') {
                ...
                if (firstUserIndex > 0) {
                    ...
                    console.warn('No user messages found in recent history, set recent messages to empty');
                    recentMessages = [];
                }
            }
            limitedMessageHistory = [...recentMessages];
            console.log(`Limiting chat history from ${messageHistory.length} to ${limitedMessageHistory.length} messages (max ${maxChatTurns} turns)`);
        }
     ...
     */
    prepareChatMessages(codebaseInfo, limitedHistory, chatMode = 'build') {
