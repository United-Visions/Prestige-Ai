...
class AppStateManager {
  ...
  private appVirtualFileSystems = new Map<number, AsyncVirtualFileSystem>();
  private appInitialStates = new Map<number, Map<string, string>>(); // Track initial file contents
   ...
   */
  async getVirtualFileSystem(app: App): Promise<AsyncVirtualFileSystem> {
   ...
   */
  private async initializeAppState(app: App): Promise<void> {
   ...
   */
  getVirtualChanges(appId: number) {
    const vfs = this.appVirtualFileSystems.get(appId);
    if (!vfs) return { virtualFiles: [], deletedFiles: [] };
   ...
   */
  resetAppToInitialState(appId: number): void {
    const vfs = this.appVirtualFileSystems.get(appId);
    if (vfs) {
   ...
   */
  async switchToApp(app: App): Promise<AsyncVirtualFileSystem> {
   ...
   */
  async syncAppToDisk(app: App): Promise<void> {
