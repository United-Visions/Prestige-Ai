...
export async function resolveAppPaths(app: App) {
  ...
  if (!appRoot.includes('prestige-ai')) {
    appRoot = await (window as any).electronAPI.path.join(prestigeRoot, app.path);
  }
  const filesPath = await (window as any).electronAPI.path.join(appRoot, 'files');
  return { appRoot, filesPath };
}

export async function resolveFilesPath(app: App) {
