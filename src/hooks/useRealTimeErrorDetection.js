 ...
 */
export const useRealTimeErrorDetection = (options = {}) => {
    const { enabled = true, onErrorDetected, onAutoFix } = options;
    const { autoFixEnabled, setCurrentReport, addProblems, currentReport } = useProblemsStore();
    const problemScanner = ProblemScannerService.getInstance();
    /**
     * Monitor streaming response and detect problems in real-time
     */
    const monitorStreamingResponse = useCallback(async (partialCode, filePath) => {
        if (!enabled || !autoFixEnabled)
            return;
        try {
            const detectedProblems = await problemScanner.detectProblemsInRealTime(partialCode, filePath);
     ...
     */
    const processCompletedResponse = useCallback(async (_fullCode, affectedFiles = []) => {
        if (!enabled)
            return;
        try {
            ...
            if (report.problems.length > 0) {
                ...
                if (autoFixEnabled && report.totalErrors > 0) {
                    const fixPrompt = problemScanner.createAutoFixPrompt(report.problems.filter(p => p.severity === 'error'));
                    onAutoFix?.(fixPrompt);
                }
            }
        }
     ...
     */
    const createAutoFixPrompt = useCallback((prioritizeErrors = true) => {
        if (!currentReport || currentReport.problems.length === 0) {
 ...
 */
export const useStreamingErrorDetection = (onAutoFixNeeded) => {
    const { monitorStreamingResponse, processCompletedResponse, shouldTriggerAutoFix, createAutoFixPrompt } = useRealTimeErrorDetection({
        enabled: true,
     ...
     */
    const processStreamChunk = useCallback(async (chunk, fullResponse, filePath) => {
     ...
     */
    const processStreamEnd = useCallback(async (finalResponse, affectedFiles = []) => {
