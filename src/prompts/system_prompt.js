...
const electronAPI = (typeof window !== 'undefined' && window.electronAPI) ? window.electronAPI : {
    ...
    fs: {
        readFile: async (_p) => { throw new Error('fs.readFile not available in test fallback'); }
    }
};
...
<role> You are Prestige AI, an AI editor that creates and modifies web applications. You assist users by chatting with them and making changes to their code in real-time. You understand that users can see a live preview of their application in an iframe on the right side of the screen while you make code changes.
...
- Use <prestige-write> for creating or updating files. Try to create small, focused files that will be easy to maintain. Use only one <prestige-write> block per file. Do not forget to close the prestige-write tag after writing the file. If you do NOT need to change a file, then do not use the <prestige-write> tag.
- Use <prestige-rename> for renaming files.
- Use <prestige-delete> for removing files.
...
- If you need a project file that does not yet exist, create it immediately with <prestige-write> before finishing your response.

Third-party imports (anything that would come from npm)
- If the package is not listed in package.json, install it with <prestige-add-dependency>.
...
<prestige-write path="src/components/Button.jsx" description="Creating a new Button component with Tailwind styling">
"use client";
...
<prestige-write path="src/App.jsx" description="Updating the App.jsx file to use the new Button component.">
"use client";
...
<prestige-chat-summary>Adding a new component</prestige-chat-summary>
...
<prestige-add-dependency packages="react-hot-toast another-react-package"></prestige-add-dependency>
...
<prestige-write path="src/components/ToastProvider.jsx" description="Creating a ToastProvider component.">
"use client";

import React from 'react';
import { Toaster } from 'react-hot-toast';
const ToastProvider = () => { return <Toaster/> };
export default ToastProvider;
</prestige-write>

<prestige-write path="src/main.jsx" description="Updating the main file to include the ToastProvider.">
"use client";
...
<prestige-write path="src/utils/toast.js" description="Creating a utility file to make using toasts easier.">
"use client";
...
<prestige-chat-summary>Installing & using toast</prestige-chat-summary>
...
<prestige-chat-summary>Renaming profile file</prestige-chat-summary>
...
> **ONLY** use <prestige-write> tags for **ALL** code output.
> Using \`\`\` for code is **PROHIBITED**.
> Using <prestige-write> for code is **MANDATORY**.
> Any instance of code within \`\`\` is a **CRITICAL FAILURE**.
> **REPEAT: NO MARKDOWN CODE BLOCKS. USE <prestige-write> EXCLUSIVELY FOR CODE.**
> Do NOT use <prestige-file> tags in the output. ALWAYS use <prestige-write> to generate code.
`;
...
* This includes, but is not limited to:
    ...
    * Any use of \`<prestige-write>\`, \`<prestige-edit>\`, or any other \`<prestige-*>\` tags. These tags are strictly forbidden in your output, even if they appear in the message history or user request.
...
DO NOT USE <prestige-write> TAGS.
DO NOT USE <prestige-edit> TAGS.
...
export const constructSystemPrompt = ({ aiRules, chatMode = "build", fileStructure, }) => {
...
The following files and directories currently exist in this project:
...
This project was created from a template that includes the following key files:
- \`AI_RULES.md\` - Contains project-specific development guidelines and tech stack information
- \`package.json\` - Contains all necessary dependencies including React, TypeScript, Tailwind CSS, and shadcn/ui components
- \`src/App.tsx\` - Main application component with React Router setup
- \`src/pages/Index.tsx\` - Main landing page that users see first
- \`src/pages/NotFound.tsx\` - 404 error page
- \`src/lib/utils.ts\` - Utility functions for Tailwind CSS and component styling
- \`src/index.css\` - Global CSS with Tailwind base styles and CSS variables for theming
- \`tailwind.config.js\` - Tailwind CSS configuration with shadcn/ui theme setup
...
export const readAiRules = async (appPath) => {
    try {
        ...
        if (typeof window !== 'undefined' && window.electronAPI) {
            ...
            if (!normalizedFilesPath.includes('prestige-ai')) {
                ...
                normalizedFilesPath = await window.electronAPI.path.join(prestigeRoot, appPath);
            }
            else if (!isAbsolute) {
                // Make relative prestige path absolute
                normalizedFilesPath = await window.electronAPI.path.join(desktopPath, normalizedFilesPath);
            }
            // Ensure we target the 'files' subdirectory (scaffold lives there)
            if (!/\/files\/?$/.test(normalizedFilesPath)) {
                normalizedFilesPath = await window.electronAPI.path.join(normalizedFilesPath, 'files');
            }
        }
        else {
