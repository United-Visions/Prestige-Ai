(function () {
  console.debug("prestige-shim.js loaded via proxy v1.0.0");
  ...
  const handleStateChangeAndNotify = (originalMethod, state, title, url) => {
    ...
    try {
      ...
      console.error(
        `[prestige-ai] Error calling original ${navigationType}: `,
        e,
      );
      window.parent.postMessage(
        {
  ...
  function sendSourcemappedErrorToParent(error, sourceType) {
    if (typeof window.StackTrace === "undefined") {
      console.error("[prestige-ai] StackTrace object not found.");
    ...
    window.StackTrace.fromError(error)
      ...
      .catch((mappingError) => {
        console.error(
          "[prestige-ai] Error during stacktrace sourcemapping:",
          mappingError,
        );
