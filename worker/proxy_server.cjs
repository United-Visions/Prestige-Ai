...
let prestigeShimContent = null;
let prestigeComponentSelectorClientContent = null;
try {
...
try {
  const prestigeShimPath = path.join(__dirname, "prestige-shim.js");
  prestigeShimContent = fs.readFileSync(prestigeShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] prestige-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read prestige-shim.js: ${error.message}`,
  );
}

try {
  const prestigeComponentSelectorClientPath = path.join(
    __dirname,
    "prestige-component-selector-client.js",
  );
  prestigeComponentSelectorClientContent = fs.readFileSync(
    prestigeComponentSelectorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] prestige-component-selector-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read prestige-component-selector-client.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the prestige shim.
  // If the prestige shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (prestigeShimContent) {
      scripts.push(`<script>${prestigeShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] prestige shim was not injected.");</script>',
      );
    }
  }
  if (prestigeComponentSelectorClientContent) {
    scripts.push(`<script>${prestigeComponentSelectorClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] prestige component selector client was not injected.");</script>',
    );
  }
